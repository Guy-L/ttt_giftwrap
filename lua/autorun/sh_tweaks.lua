...
hook.Add("AcceptInput", ENT_KILL_INPUT_HOOK, function(ent, input, activator, caller, value)
    if (input == "kill" or input == "Kill") and IsValid(ent) and IsValid(ent:GetNW2Entity("WrappedByGift")) or ent._DontKill then
...
local initChanges = {
    ...
    {   name = "lethal_mine_wrap_fix",
        addon = "Lethal Mine", icon = "materials/vgui/ttt/icon_lethalmine.png",
        desc = "Prevent Lethal Mines exploding in giftbox", realm = ChangeRealm.SERVER,
    ...
    {   name = "smoke_wrap_client_fix",
        addon = "Smoke Grenade", icon = "vgui/ttt/icon_smokegrenade",
        desc = "Prevent smoke from appearing client-side when grenade is wrapped", realm = ChangeRealm.CLIENT,
        identifier = "ttt_smokegrenade_proj", category = ChangeCategory.SENT,
        original_keys = {"Explode"},
        apply = function(sent, og)

            sent.Explode = function(self, tr)
                if self:GetNWBool("GWPhysStasis") then return end
                og.Explode(self, tr)
            end
        end
    },

    {   name = "fix_custom_worldmodels",
        addon = "TTT2 (Base)", icon = "vgui/ttt/icon_halp",
        desc = "Fix certain SWEP worldmodels being visually frozen after exiting the client's PVS", realm = ChangeRealm.CLIENT,
        identifier = nil, category = ChangeCategory.None,
        apply = function()

            -- Tweak can be removed once https://github.com/TTT-2/TTT2/issues/1874 is fixed
            local worldmodel_fix_list = {
            ...
            for _, swepID in ipairs(worldmodel_fix_list) do
    ...
    {   name = "override_sent_markervision",
        addon = "TTT2 (Base)", icon = "vgui/ttt/icon_halp",
        desc = "Extend markervision logic for certain entities to make them not show while wrapped", realm = ChangeRealm.CLIENT,
        identifier = nil, category = ChangeCategory.None,
        apply = function()

            local mv_hook_list = {
                sent_greendemon              = "HUDDrawMarkerVisionGreenDemon",
                weapon_ttt_propexploderv2    = "HUDDrawMarkerVisionPropExploder",
                npc_barnacle                 = "BarnacleMarkerVisionDisplay",
                --ttt_c4                       = "HUDDrawMarkerVisionC4", -- exception
                ttt_conmine                  = "HUDDrawMarkerVisionConmine",
                ent_ttt_fan                  = "FanMarkerVisionDisplay",
                sent_greendemon_box          = "HUDDrawMarkerVisionGreenDemonBox",
                item_lethal_company_landmine = "LethalMineMarkerVisionDisplay",
                ttt_slam_satchel             = "SLAMMarkerVisionDisplay",
                ttt_poison_station           = "PoisonStationMarkerVisionDisplay",
                ttt_radio                    = "HUDDrawMarkerVisionRadio",
                ttt_soap                     = "HUDDrawMarkerVisionSoap",
                ttt_springmine               = "HUDDrawMarkerVisionSpringMine",
            }

            for sentID, hookName in pairs(mv_hook_list) do
                local ogHook = hook.GetTable()["TTT2RenderMarkerVisionInfo"][hookName]

                if ogHook then
                    hook.Add("TTT2RenderMarkerVisionInfo", hookName, function(mvData)
...
hook.Add("PostInitialize", INIT_FIXES_HOOK, function()
    ...
    print("Loaded all " ..#initChanges.. " third-party tweaks.")
end)
